...
#define PANIC(fmt, ...)                                                  \
    do {                                                                 \
        printf("PANIC: %s:%d: " fmt, __FILE__, __LINE__, ##__VA_ARGS__); \
        while (1)                                                        \
            ;                                                            \
    } while (0);
...
/* nページ分のメモリを動的に割り当てる */
/* 頭から割り当てていくため，そのうちメモリリークするがまぁいいか...となっている */
...
void yield(void){
    ...
    for(int i = 0; i < PROCS_MAX; i++){
        ...
        if(proc->state == PROC_RUNNABLE && proc->pid > 0){ // idle_procはpidが-1なので除外
            next = proc;
            break;
        }
    }
...
struct process *create_process(uint32_t pc){
    ...
    // switch_context()で復帰できるようにスタックに呼び出し先保存レジスタを積む
